...
bool UiObject::MouseInputToObject(sf::Event::EventType const& event_type,
	...
	sf::Vector2f mouse = sf::Vector2f((float)mouse_position.x, 
		(float)mouse_position.y) - this->GetCoordinateCentre();
...
void UiObject::SetStartCoorditateLeft() { centring_x = Object_centring::LEFT; }

void UiObject::SetStartCoorditateRight() { centring_x = Object_centring::RIGHT; }

void UiObject::SetStartCoorditateCentre() { centring_x = Object_centring::CENTER; }

void UiObject::SetStartCoorditateTop() { centring_y = Object_centring::TOP; }

void UiObject::SetStartCoorditateMiddle() { centring_y = Object_centring::MIDDLE; }

void UiObject::SetStartCoorditateBottom() { centring_y = Object_centring::BOTTOM; }
...
void UiObject::Draw(sf::RenderWindow& window) {
	if (Anchor_object_ != nullptr)
		...
		if (centring_x == Object_centring::LEFT) {
			coordinate_x = this->GetCamera()->getCenter().x
				- this->GetCamera()->getSize().x / 2.f
				+ coordinate_centre_.x;
		}
		else if (centring_x == Object_centring::CENTER) {
			coordinate_x = this->GetCamera()->getCenter().x
				+ coordinate_centre_.x;
		}
		else if (centring_x == Object_centring::RIGHT) {
			coordinate_x = this->GetCamera()->getCenter().x
				+ this->GetCamera()->getSize().x / 2.f
				- coordinate_centre_.x;
		}
		if (centring_y == Object_centring::TOP) {
			coordinate_y = this->GetCamera()->getCenter().y
				- this->GetCamera()->getSize().y / 2.f
				+ coordinate_centre_.y;
		}
		else if (centring_y == Object_centring::MIDDLE) {
			coordinate_y = this->GetCamera()->getCenter().y
				+ coordinate_centre_.y;
		}
		else if (centring_y == Object_centring::BOTTOM) {
			coordinate_y = this->GetCamera()->getCenter().y
				+ this->GetCamera()->getSize().y / 2.f
				- coordinate_centre_.y;
		}
	...
	else this->SetCoordinate(coordinate_centre_);
