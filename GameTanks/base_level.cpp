...
BaseLevel::BaseLevel() {
	...
	count_id_objects_ = 1;
	size_level_border_ = 0;
	min_distance_respawn_to_Static_objects_ = 0.001f; //% size level
	min_distance_respawn_to_Enemy_objects_ = 0.005f; //% size level
	min_distance_respawn_to_Players_objects_ = 0.01f; //% size level
	min_distance_respawn_to_Shot_objects_ = 0.0000010f; //% size level
}
...
bool BaseLevel::AddStaticObject(GameObject* Static_objects,
	...
	if (Static_objects != nullptr 
		...
		Static_objects->SetGameType(int(Object_type::STATIC));
...
bool BaseLevel::AddEnemyObject(TankObject* Enemy_objects,
	...
	if (Enemy_objects != nullptr
		...
		Enemy_objects->SetGameType(int(Object_type::ENEMY));
...
bool BaseLevel::AddPlayerObject(TankObject* Player_objects,
	...
	if (Player_objects != nullptr
		...
		Player_objects->SetGameType(int(Object_type::PLAYER));
...
bool BaseLevel::AddShotObject(MovebleObject* Shot_objects) {
	if (Shot_objects != nullptr) {
		...
		Shot_objects->SetGameType(int(Object_type::SHOT));
...
bool BaseLevel::SetBonusObject(GameObject* Bonus_object,
	...
	if (Bonus_object != nullptr
		...
		Bonus_object_->SetGameType(int(Object_type::BONUS));
...
int BaseLevel::GetPlayerCount() { return Players_objects_.size(); }

GameObject* BaseLevel::GetObjectById(int const& id_object) {
	for (auto item : All_objects_)
		if (item->GetIdObject() == id_object) return item;
	return nullptr;
}
...
void BaseLevel::SetBorderTexture(std::string texture_address, 
	...
	if (size_level_width_ <= 0) size_level_width_ = (int)SCREEN_RESOLUTION_X;
	if (size_level_height_ <= 0) size_level_height_ = (int)SCREEN_RESOLUTION_Y;
}
...
sf::Packet BaseLevel::GetPacketToSendAllClient(int const& player_number,
	...
	else if (Packet_send_changes_[player_number].getDataSize() > 0)
			Paket = Packet_send_changes_[player_number];
	return Paket;
}

void BaseLevel::RecvPacketFromServer(sf::Packet& Packet) {
	if (Packet.getDataSize() > 0) Packets_recv_.push_back(Packet);
}
...
void BaseLevel::UnpackingPacket(sf::Packet& Packet) {
	...
	for (auto item : All_objects_)
		if (item->GetCoordinateCentre().x <= -100.f 
			|| item->GetCoordinateCentre().y <= -100.f) item->StopAllSounds();
}
...
bool BaseLevel::UpdateState(float& game_timer) {
	...
	for (auto const item : Dies_objects_)
		if (item->GetTimeToRespawn() == 0) {
			item->RestoreLife();
			item->CollisionOn();
			if (this->RespawnObject(item)) Dies_objects_.remove(item);
		}
		...
		for (i = 0; i < (int)Players_objects_.size(); i++) {
			if ((item->GetCoordinateCentre().x + item->GetWidthSprite() > 
					...
					item->GetCoordinateCentre().y - item->GetHeightSprite() < 
						Players_objects_[i]->GetCoordinateCentre().y 
							+ SCREEN_RESOLUTION_Y) 
					|| Players_objects_[i]->GetGameType() == int(Object_type::PLAYER)) {
...
bool CompareObjects(GameObject* first, GameObject* second) {
...
void BaseLevel::CalculateCollisionOnLevel() {
	...
	while (recalc_all) {
		recalc_all = false;
		All_objects_.sort(CompareObjects);
		CalculateCollisionObject(recalc_all);
	}
...
void BaseLevel::CalculateCollisionObject(bool& recalc_all) {
	for (auto it = All_objects_.begin(); it != All_objects_.end(); ++it) {
		if ((*it)->GetGameType() == int(Object_type::PLAYER) &&
		...
		if ((*it)->GetGameType() == int(Object_type::SHOT) || (*it)->ObjectInRangeLevel(
			...
			for (; it2 != All_objects_.end(); ++it2) {
				if ((((*it)->GetCoordinateCentre().x + max_safe_distance) >=
					...
					if ((*it)->SafeDistanceToCollision((*it2)) <= 0) {
						if (!(*it)->Collision((*it2))) {
							if ((*it)->GetGameType() == int(Object_type::SHOT) &&
								(*it2)->GetGameType() == int(Object_type::SHOT)) {
							...
							else if ((*it)->GetGameType() != int(Object_type::SHOT) && 
								(*it2)->GetGameType() != int(Object_type::SHOT)) {
								(*it)->RestorePreviousState();
								if ((*it)->GetGameType() == int(Object_type::ENEMY))
									((MovebleObject*)(*it))->SetDistanceMove(-40);
								(*it2)->RestorePreviousState();
								if ((*it2)->GetGameType() == int(Object_type::ENEMY))
								...
								if (object->GetLifeLevel() != 0) {
									...
									if (shot->GetPerrent() != nullptr) {
										float procent = (float)shot->GetLifeLevel() /
									...
									//Work by shooting:
									object->SetLifeLevel(-shot->GetLifeLevel(), true);
			...
			if ((*it)->GetGameType() != int(Object_type::STATIC)) {
				(*it)->RestorePreviousState();
				if ((*it)->GetGameType() == int(Object_type::ENEMY))
					((MovebleObject*)(*it))->SetDistanceMove(0);
				(*it)->StartAudioAction("collision");
...
bool BaseLevel::SafePointSpawn(GameObject* Game_object) {
	...
	int i;
	//Check Game_object's position relative to Static_objects_
	...
	//Check Game_object's position relative to Enemy_objects_
	...
	//Check Game_object's position relative to Players_objects_
	...
	//Check Game_object's position relative to Shot_objects_
...
bool BaseLevel::RespawnObject(GameObject* Game_object) {
	for (int i = 0; i < 10000000; i++) {
		...
		if (this->SafePointSpawn(Game_object)) return true;
	}
	std::cout << "Error occurred, more than 10000000 respawn!" << std::endl;
	Game_object->RestorePreviousState();
	return false;
}

bool BaseLevel::CheckExitLevel(sf::Packet& Result_level) { return false; }

BaseLevel::~BaseLevel() {
	for (auto object : Ui_objects_) delete object;
	for (auto object : Static_objects_) delete object;
	for (auto object : Enemy_objects_) delete object;
	for (auto object : Players_objects_) delete object;
	for (auto object : Shot_objects_) delete object;
}
